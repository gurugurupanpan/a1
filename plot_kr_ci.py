...
ax.set_xlabel('Point Estimate Value', fontsize=12, fontweight='bold')
...
for i, (bar, pe) in enumerate(zip(bars, point_estimates)):
    label_x = pe + (upper_errors[i] if pe > 0 else -lower_errors[i]) + (50 if pe > 0 else -50)
    ax.text(label_x, i, f'{pe:.1f}',
            va='center', ha='left' if pe > 0 else 'right',
...
for i, (param, pe, cl, cu) in enumerate(zip(parameters, point_estimates, ci_lower, ci_upper)):
    # Draw CI line
    ax2.plot([cl, cu], [i, i], 'o-', linewidth=3, markersize=8,
             color=colors[i], alpha=0.6, label=param)
    # Draw point estimate
    ax2.plot(pe, i, 'D', markersize=10, color=colors[i],
             markeredgecolor='black', markeredgewidth=1.5)

    # Add text annotations
    ax2.text(cu + 30, i, f'{param}\n[{cl:.1f}, {cu:.1f}]',
             va='center', ha='left', fontsize=9)

ax2.set_yticks(y_pos)
ax2.set_yticklabels(parameters, fontsize=11)
ax2.set_xlabel('Parameter Value', fontsize=12, fontweight='bold')
ax2.set_title('Krinsky-Robb 95% Confidence Intervals\n(Diamond = Point Estimate, Line = CI Range)',
              fontsize=14, fontweight='bold', pad=20)
