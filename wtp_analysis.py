...
def plot_confidence_intervals(df, output_file='wtp_confidence_intervals.png'):
    ...
    # Reorder the dataframe: LFpref=1 (bottom), LFpref=0 (middle), Overall (top)
    # Original order in CSV: Overall, LFpref=0, LFpref=1
    # Reverse to get: LFpref=1, LFpref=0, Overall (from bottom to top)
    df_plot = df.iloc[::-1].reset_index(drop=True)

    categories = df_plot['Category'].tolist()
    means = df_plot['Mean'].tolist()
    ...
    for i, (cat, mean, lower, upper) in enumerate(zip(categories, means, ci_lower, ci_upper)):
        ...
        # After reordering: i=0 (LFpref=1, bottom), i=1 (LFpref=0, middle), i=2 (Overall, top)
        if i == 2:  # Top row (Overall) - place text below with larger offset
            ax.text(mean, i - 0.6, f'Mean: {mean:.2f}\nCI: [{lower:.3f}, {upper:.3f}]',
                    ha='center', va='top', fontsize=27,
                    bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
        else:  # Bottom and middle rows (LFpref=1 and LFpref=0) - place text above with larger offset
            ax.text(mean, i + 0.6, f'Mean: {mean:.2f}\nCI: [{lower:.3f}, {upper:.3f}]',
